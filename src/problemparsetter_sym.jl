...
"""
function ProblemParSetter_sym(state_syms::NTuple{NS, Symbol},
    par_syms::NTuple{NP, Symbol},
    popt_syms::NTuple{NO, Symbol},
    ::Val{allow_missing_popt} = Val(true);
    it::Type{IT} = Int64) where {NO, NS, NP, IT, allow_missing_popt}
    ...
    map_posu_to_posopt = Dict(pos => posopt for
                              (posopt, (source, parname, pos)) in enumerate(optinfo)
                              if source == :state)
    ...
    if allow_missing_popt && (NO != length(optinfo))
        ...
        ps = ProblemParSetter_sym{NS, NP, length(optinfo), IT}(optinfo,
            statemap,
            parmap,
            state_syms,
            par_syms)
        ...
        ps = ProblemParSetter_sym{NS, NP, NO, IT}(optinfo,
            statemap,
            parmap,
            state_syms,
            par_syms)
    ...
    symbols_paropt(ps) == popt_syms_in ||
        error("Expected states to optimize before parameters to optimize. But got $popt_syms_in.")
    ps
end

function ProblemParSetter_sym(state_names, par_names, popt_names)
    # not type-stable
    ProblemParSetter_sym(Tuple(i for i in symbol.(state_names)),
...
function label_paropt(ps::ProblemParSetter_sym, popt::SVector)
...
function name_state(ps::ProblemParSetter_sym, state::AbstractVector)
    NamedArray(state, (collect(symbols_state(ps)),))
end
function name_par(ps::ProblemParSetter_sym, par::AbstractVector)
    NamedArray(par, (collect(symbols_par(ps)),))
end
function name_paropt(ps::ProblemParSetter_sym, paropt::AbstractVector)
