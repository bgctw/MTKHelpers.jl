...
function ProblemParSetter_sym(
    state_syms::NTuple{NS,Symbol},
    par_syms::NTuple{NP,Symbol},
    popt_syms::NTuple{NO,Symbol},
    ::Val{allow_missing_popt} = Val(true);
    it::Type{IT} = Int64,
) where {NO,NS,NP,IT,allow_missing_popt}
    posu_orig =
        Dict(parname => convert(IT, pos) for (pos, parname) in enumerate(state_syms))
    posp_orig = Dict(parname => convert(IT, pos) for (pos, parname) in enumerate(par_syms))
    #
    optinfo_u = Tuple(
        (:state, statesym, posu_orig[statesym]) for
        statesym in popt_syms if haskey(posu_orig, statesym)
    )
    optinfo_p = Tuple(
        (:par, parsym, posp_orig[parsym]) for
        parsym in popt_syms if haskey(posp_orig, parsym)
    )
    optinfo = (optinfo_u..., optinfo_p...)
    #    
    map_posu_to_posopt = Dict(
        pos => posopt for
        (posopt, (source, parname, pos)) in enumerate(optinfo) if source == :state
    )
    map_posp_to_posopt = Dict(
        pos => posopt for
        (posopt, (source, parname, pos)) in enumerate(optinfo) if source == :par
    )
    # 
    statemap = NTuple{NS}((get(map_posu_to_posopt, posu, 0) for posu = one(IT):NS))
    parmap = NTuple{NP}((get(map_posp_to_posopt, posp, 0) for posp = one(IT):NP))
    ...
    if allow_missing_popt && (NO != length(optinfo))
        ...
        @warn("missing optimization parameters in system: " * join(miss, ", "))
...
function count_state(::ProblemParSetter_sym{NS}) where {NS}
    NS
end,
function count_par(::ProblemParSetter_sym{NS,NP}) where {NS,NP}
    NP
end,
function count_paropt(::ProblemParSetter_sym{NS,NP,NO}) where {NS,NP,NO}
    NO
end
...
function symbols_state(ps::ProblemParSetter_sym)
    ps.statesyms
end,
function symbols_par(ps::ProblemParSetter_sym)
    ps.parsyms
end,
...
function update_statepar(ps::ProblemParSetter_sym, popt, u0::TU, p::TP) where {TU,TP}
    ...
    u0g = (ps.statemap[i] == 0 ? u0[i] : popt[ps.statemap[i]] for i = 1:length(u0))
    ...
    pg = (ps.parmap[i] == 0 ? p[i] : popt[ps.parmap[i]] for i = 1:length(p))
...
function get_paropt(ps::ProblemParSetter_sym{NS,NP,NO}, u0, p) where {NS,NP,NO}
    t0 = NTuple{NO}(((first(t) == :par) ? p[last(t)] : u0[last(t)] for t in ps.optinfo))
...
"""
function label_state(ps::ProblemParSetter_sym, u::AbstractVector)
    LArray{symbols_state(ps)}(u)
end,
function label_par(ps::ProblemParSetter_sym, par::AbstractVector)
    LArray{symbols_par(ps)}(par)
end,
function label_paropt(ps::ProblemParSetter_sym, popt::AbstractVector)
    LArray{symbols_paropt(ps)}(popt)
end
...
label_paropt(ps::ProblemParSetter_sym, popt::SVector) =
    SLVector(label_paropt(ps, Tuple(popt)))
...
name_state(ps::ProblemParSetter_sym, state::AbstractVector) =
    NamedArray(state, (collect(symbols_state(ps)),))
name_par(ps::ProblemParSetter_sym, par::AbstractVector) =
    NamedArray(par, (collect(symbols_par(ps)),))
name_paropt(ps::ProblemParSetter_sym, paropt::AbstractVector) =
    NamedArray(paropt, (collect(symbols_paropt(ps)),))
