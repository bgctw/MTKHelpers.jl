...
ps = ps1 = ProblemParSetterComp1(u1,p1,popt1)
...
@testset "warning on missing symbols" begin
    ...
    psw = @test_logs (:warn,r"missing optimization parameters") ProblemParSetterComp1(Axis(state_syms), Axis(par_syms), Axis(popt_syms))
...
@testset "warning on duplicate symbols" begin
    ...
    par_syms = keys(p1)
    popt_syms = (:k_L,)
    psw = @test_logs (:warn,r"to be distinct") ProblemParSetterComp1(Axis(state_syms), Axis(par_syms), Axis(popt_syms))
    p2 = p1 .* 2
...
@testset "update ODEProblem optimized parameters" begin
    ...
    #sol = solve(prob)
    #sol[end]
    pset = ProblemParSetterComp1(Axis(keys(u0)),Axis(keys(p)),Axis((:u1,:p2)))
...
@testset "gradient with Vector" begin
    fcost = (popt) -> begin
        u0, p = update_statepar(pset, popt, u1, p1)
        d = sum(get_paropt(pset, u0, p))
        d*d
    end
    fcost(popt)
...
@testset "name_paropt" begin
    ...
    #psr = @inferred ProblemParSetterComp1(Axis(keys(u1)),Axis(keys(p1)),Axis(frandsym()))
    ftmp = (poptnames) -> ProblemParSetterComp1(Axis(keys(u1)),Axis(keys(p1)),Axis(poptnames))
